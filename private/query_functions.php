<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    else if (preg_match('/\A[A-Za-z\s\-,\.\']+\Z/', $state['name']) == 0) {
      $errors[] = "State name has invalid characters. Only letters, spaces, and symbols (-,.') are allowed.";
    }
    ...
    // My Custom Validation
      ...
      $errors[] = "State code has invalid characters. Only capital letters are allowed.";
    }

    if (is_blank($state['country_id'])) {
      $errors[] = "Country ID cannot be blank.";
    }
    else if (!has_length($state['country_id'], array('min' => 1, 'max' => 255))) {
      $errors[] = "Country ID must be between 1 and 255 characters.";
    }
    // My Custom Validation
    else if (preg_match('/\A[0-9]+\Z/', $state['country_id']) == 0) {
      $errors[] = "Country ID has invalid characters. Only numbers are allowed.";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    // My Custom Validation
    else if (preg_match('/\A[A-Za-z\s\-,\.\']+\Z/', $territory['name']) == 0) {
      $errors[] = "Territory name has invalid characters. Only letters, spaces, and symbols (-,.') are allowed.";
    }

    if (is_blank($territory['position'])) {
      $errors[] = "Position cannot be blank.";
    }
    else if (!has_length($territory['position'], array('min' => 1, 'max' => 255))) {
      $errors[] = "Position must be between 1 and 255 characters.";
    }
    // My Custom Validation
    else if (preg_match('/\A[0-9]+\Z/', $territory['position']) == 0) {
      $errors[] = "Position has invalid characters. Only numbers are allowed.";
    }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    else if (preg_match('/\A[A-Za-z0-9_@\.\-]+\Z/', $salesperson['email']) == 0)  {
    ...
    else if (preg_match('/\A[0-9()\- ]+\Z/', $salesperson['phone']) == 0) {
      $errors[] = "Phone number must contain only the whitelisted characters: 0-9, spaces, and ()-.";
    }
